...
def create_CAPEC_vectors():
    ...
    for x in range(1, 547):
        capec_lda = gensim.models.LdaModel.load("LDA/capec_lda_" + str(capec_num))
        corpus2 = corpora.MmCorpus("CORPUSES/capec_corpus_" + str(capec_num))
...
def get_CAPEC_IDs(capec_list):
    CAPEC_IDs = pd.read_csv("Comprehensive CAPEC Dictionary.csv",usecols=["ID"])
    ...
    print(len(list_with_topics_and_capec_ids))
...
srs_lda_filename = "logistics_srs_lda"
srs_corpus_filename = "logistics_srs_corpus"
capec_num = 1
...
pandas_frame.to_csv(csv_filename,encoding="utf-8",index=False)
